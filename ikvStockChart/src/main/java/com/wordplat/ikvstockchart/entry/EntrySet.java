...
public class EntrySet {
    ...
    public List<Entry> subEntry(int start,int end) {
    ...
    private void computeMANeed() {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            if (i >= 4) {
                ma4 -= entries.get(i - 4).getClose();
                entry.setMa4(ma4 / 4f);

            } else {
                entry.setMa4(ma4 / (i + 1f));

            }

            if (i >= 9) {
                ma9 -= entries.get(i - 9).getClose();
                entry.setMa9(ma9 / 9f);

            } else {
                entry.setMa9(ma9 / (i + 1f));

            }

            if (i >= 18) {
                ma18 -= entries.get(i - 18).getClose();
                entry.setMa18(ma18 / 18f);
            } else {
                entry.setMa18(ma18 / (i + 1f));
            }

            if (i >= 50) {
                ma50 -= entries.get(i - 50).getClose();
                entry.setMa50(ma50 / 50f);
            } else {
                entry.setMa50(ma50 / (i + 1f));
            }
        }
    }
    /**
    ...
    private void computeRaiseRate() {
        for(int i=0; i<entries.size()-1;i++){
            Entry entry1 = entries.get(i);
            Entry entry2 = entries.get(i+1);
            float diff = entry2.getClose() - entry1.getClose();
            entry2.setRate(diff/entry1.getClose()*100);
        }
    }
