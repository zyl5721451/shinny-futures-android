...
public class ViewUtils {
     ...
     * 设置 蜡烛图画笔的颜色和是否空心
     */
    public static Entry setUpLineCandlePaint(Paint candlePaint, EntrySet entrySet, int currentEntryIndex, SizeColor sizeColor) {
        Entry entry = entrySet.getEntryList().get(currentEntryIndex);

        if(entry.getEntry_type().equals(FIRST_ENTRY_TYPE.UPING)||entry.getEntry_type().equals(FIRST_ENTRY_TYPE.UPINGSIGNAL)) {
            candlePaint.setColor(sizeColor.getIncreasingColor());
        }else if(entry.getEntry_type().equals(FIRST_ENTRY_TYPE.DONWING)||entry.getEntry_type().equals(FIRST_ENTRY_TYPE.DOWNINGSIGNAL)) {
            candlePaint.setColor(sizeColor.getDecreasingColor());
        }else{
